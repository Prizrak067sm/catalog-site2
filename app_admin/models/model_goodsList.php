<?php
    class Model_goodsList extends Model
    {
        ...
        function go_query()
        {
            $query = 'SELECT id, `Название книги`, `Наличие (кол-во)`, `Доступно для заказа` FROM goods WHERE Активность>0';   // Строка запроса на выборку записей товаров со значением поля активность больше нуля.
            $this->result_query = mysqli_query($this->link, $query);   // Результат запроса.
            $this->rows_count = mysqli_num_rows($this->result_query);   // Количество всех записей выборки из таблицы.
        }

        // Возвращает количество страниц. Необходимо для вьюшки. Число зависит от количества записей в таблице.
        function get_page_count()
        {
            $sql = "SELECT id FROM goods WHERE activ>0";
